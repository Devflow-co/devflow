...
const LinearSystemConfigSchema = z.object({
  statuses: z.object({
    ...
    toCode: z.string().default('To Code'),
    codeInProgress: z.string().default('Code In Progress'),
    codeReview: z.string().default('Code Review'),
    codeReady: z.string().default('Code Ready'),
    codeFailed: z.string().default('Code Failed'),
  }),
...
export function loadConfig(): DevFlowConfig {
  ...
  const linearConfig = LinearSystemConfigSchema.parse({
    statuses: {
      ...
      // Code Generation (Phase 4)
      toCode: process.env.LINEAR_STATUS_TO_CODE || 'To Code',
      codeInProgress: process.env.LINEAR_STATUS_CODE_IN_PROGRESS || 'Code In Progress',
      codeReview: process.env.LINEAR_STATUS_CODE_REVIEW || 'Code Review',
      codeReady: process.env.LINEAR_STATUS_CODE_READY || 'Code Ready',
      codeFailed: process.env.LINEAR_STATUS_CODE_FAILED || 'Code Failed',
    },
