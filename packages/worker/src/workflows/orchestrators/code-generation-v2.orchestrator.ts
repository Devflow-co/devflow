/**
 * Code Generation Orchestrator V2 - Modularized Version
 *
 * This is the refactored version using sub-workflows for each step.
 * The orchestrator acts as a simple router that coordinates sub-workflows.
 *
 * Sub-workflows:
...
/**
 * Code Generation Orchestrator - Modularized Version
 *
 * Coordinates sub-workflows for Phase 4 code generation.
 * Signal handling for V3 interactive features remains in this orchestrator.
 */
export async function codeGenerationOrchestratorV2(
  ...
  const { logWorkflowFailure } = proxyActivities<typeof progressActivities>({
  ...
  try {
    ...
    if (features.enableAmbiguityDetection) {
      ...
      if (ambiguityResult.waitingForResponse && ambiguityResult.questionId) {
        ...
        const response = await waitForQuestionResponse(timeoutHours);
